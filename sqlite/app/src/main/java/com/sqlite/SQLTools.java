...
public class SQLTools {
    ...
    public static SQLiteDatabase opendatabase(Context context) {
        ...
        File file = new File(context.getFilesDir(), "local.db");
        // 当文件不存在的时候：才去拷贝，已经存在的不再去拷贝了。
        if (!file.exists()) {
            AssetManager assetManager = context.getAssets();
            try {
                // 获取输入流
                InputStream is = assetManager.open("local.db");
    ...
    //hashset 避免假如重复的对象
    static HashSet<Activity> set = new HashSet<>();

    public static void pullStatckActivity(Activity activity) {
        set.add(activity);
    }


    public static void clearStackActivity() {
        for (Activity a : set) {
            a.finish();
        }
    }
    ...
    //存储country的信息
    public static City country = new City();
    public static City province = new City();
    public static City city = new City();
    public static City region = new City();
